...
CardV = enum.IntEnum('CardV', 'J 2 3 4 5 6 7 8 9 T Q K A')
...
CardV.from_string = card_from_string
...
class HandBase:
    ...
    def __repr__(self):
        ...
        return f'{self.__class__.__name__}(cards={"".join(c._name_ for c in self.cards)}, type={self.type._name_}, bid={self.bid})'
...
class HandV(HandBase):
    ...
    def type(self):
        ...
        if (CardV.J in card_counts and len(card_counts) > 1):
            j_count = card_counts[CardV.J]
            del card_counts[CardV.J]
            argmax = max(card_counts, key=card_counts.get)
            card_counts[argmax] += j_count
...
def read_inputs(example=0) -> tuple[list[Hand], list[HandV]]:
    ...
    for line in data:
        ...
        handvs.append(HandV(tuple(CardV.from_string(c) for c in cards), int(bid)))
...
def main(args):
    ...
    hands, handvs = read_inputs(args.example)
    ranks = part1(hands)
    logging.info(f'Part 1: {sum(map(lambda r, h: r * h.bid, ranks, hands))} ({", ".join(str(rh) for rh in zip(ranks, hands))})')
    rankvs = part1(handvs)
    logging.info(f'Part 2: {sum(map(lambda r, h: r * h.bid, rankvs, handvs))} ({", ".join(str(rh) for rh in zip(rankvs, handvs))})')
